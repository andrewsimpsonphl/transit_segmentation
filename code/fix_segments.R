...
final_segments <- segments %>% mutate(fromto = toString(new_fromto)) %>% select(-linknumber, -new_fromto, -to, -from, 
    -tofrom, -FID, -FID_, -round, -times_used, -CreationDate, -Creator, -EditDate, -Editor) %>% left_join(links_df, 
    by = c(fromto = "combo"))
