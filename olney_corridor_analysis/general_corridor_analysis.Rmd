...
These plots show the variation in density of run times within each hour of the day. Narrower, higher curves indicate a concentration of run times; in other words, most run times within the hour were about the same. Wider, flatter curves indicate the opposite, that there was large variation in the sampled run times.
...
max_dur <- lubridate::as.duration(max(dat$run))
min_dur <- lubridate::as.duration(min(dat$run))

f <- function(route_num, direction) {
  p <- plot_joyplot_runtimes(dat, route_num = route_num, direction = direction, pattern_list = primary_patterns[[1]], corridor_name = full_corridor_dat$corridor_name[[1]], min_dur, max_dur)
...
table_2 <- full_stop_data$daily_stop_analytics[[1]] %>% ungroup() %>% select(-c(avg_speed, stop_lat, stop_lon))
...
reactable(table_2,
    ...
    # ),
      ...
      routes=colDef(name="Route",width=100),
      stop_id=colDef(name="Stop ID"),stop_name=colDef(name="Stop Name"),
      total_trips = colDef(name="Total Trips"),
      avg_headway = colDef(name="Average Headway"),
      total_ons = colDef(name= "Total Ons",aggregate = "sum", format = colFormat(digits = 0)),
      total_offs = colDef(name="Total Offs",aggregate = "sum", format = colFormat(digits = 0)),
      avg_load = colDef(name="Average Load",format = colFormat(digits = 0)),
      avg_dwell_observed = colDef(name="Avg Observed Dwell",format = colFormat(digits = 2)),
      avg_dwell_estimated = colDef(name="Avg Estimated Dwell",format = colFormat(digits = 2)),
      avg_dwell_hybrid = colDef(name="Avg Hybrid Dwell",format = colFormat(digits = 2)),
      avg_dwell_per_pass_hybrid = colDef(name="Avg Hybrid Dwell per Passenger",format = colFormat(digits = 2))
    )
)
...
reactable(
  ...
  columns = list(
    ...
    #   headerStyle = list(borderRight = "1px solid #eee")
    # ),
    route_id=colDef(name="Route",width=100),direction_id=colDef(name="Direction",width=150),
    stop_lat=colDef(name="Stop Latitude",width=100),stop_lon=colDef(name="Stop Longitude",width=100),
    stop_id=colDef(name="Stop ID"),stop_name=colDef(name="Stop Name"),
    total_trips = colDef(name="Total Trips",aggregate = "max"),
    avg_headway = colDef(name="Average Headway",aggregate = "mean", format = colFormat(digits = 1)),
    total_ons = colDef(name= "Total Ons",aggregate = "sum", format = colFormat(digits = 1)),
    total_offs = colDef(name="Total Offs",aggregate = "sum", format = colFormat(digits = 1)),
    avg_load = colDef(name="Average Load",aggregate = "mean", format = colFormat(digits = 1)),
    avg_dwell_observed = colDef(name="Avg Observed Dwell",aggregate = "mean", format = colFormat(digits = 1)),
    avg_dwell_estimated = colDef(name="Avg Estimated Dwell",aggregate = "mean", format = colFormat(digits = 1)),
    avg_dwell_hybrid = colDef(name="Avg Hybrid Dwell",aggregate = "mean", format = colFormat(digits = 1)),
    avg_dwell_per_pass_hybrid = colDef(name="Avg Hybrid Dwell per Passenger",aggregate = "mean", format = colFormat(digits = 1))
  ),
