...
ordered_stop_dat <- full_stop_data$stop_trip_dat[[1]] %>% 
  ...
  summarise(directions = list(unique(direction_id))) %>% 
...
``` {r stop_route_daily_table, include=TRUE, message=FALSE}
...
``` {r stop_plot, include=TRUE, message=FALSE, fig.height = 10}
plot_stops(full_stop_data, ordered_stop_dat)
