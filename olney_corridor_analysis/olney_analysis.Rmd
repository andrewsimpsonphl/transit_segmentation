...
#primary_patterns <- code_patterns(full_corridor_dat$dat[[1]]) %>% filter(pattern_type == "primary") %>% select(pattern_id) %>% as.list()
routes <- unique(full_corridor_dat[[2]])
dirs <- unique(full_corridor_dat[[3]])

for(l in dirs){
  d<-l[[1]]
  for(i in routes){
    r<-i[[1]]
    print(plot_joyplot_runtimes(full_corridor_dat, route_num = r, direction = d, pattern_list = primary_patterns[[1]]))
  }
}
