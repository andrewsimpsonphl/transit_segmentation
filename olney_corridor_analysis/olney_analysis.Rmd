...
trip_dat <- (find_trip_dat_v2(apc_data, stop_list)) %>% 
            rowwise() %>% 
            mutate(dwell_est = case_when(     #I have no idea what's going on here so I'm going to try to create a simple variable below
                   ...
                   dwell_calc = case_when(
                     ons_total>offs_total ~ (ons_total-offs_total)*4.8,
                     ons_total<=offs_total ~ offs_total*4.8)
            ) %>% 
