...
plot_speed_by_route_dir <- function(df,id) {
  a <- as.data.frame(subset(df$hourly_route_direction_analytics, subcorridor_id==id))
  p<- ggplot(a, aes(x = trip_hour, y = avg_segment_speed, color=direction_id)) + 
        geom_line() +
        geom_point(size=1.5) + 
        ...
        labs(title = "Hourly Speed by Route/Direction",
             subtitle = subset(df$subcorridor_name, subcorridor_id==id)) +
...
plot_ridership_by_route_dir <- function(df, id) {
  a <- as.data.frame(subset(df$hourly_route_direction_analytics, subcorridor_id==id))
  p <- ggplot(a, aes(x = trip_hour, y = daily_ridership, fill=direction_id)) + 
...
plot_hourly_speed <- function(df,id){
    p <- ggplot(as.data.frame(subset(df$hourly_analytics, subcorridor_id==id))) + 
...
# plot daily ridership for a specified subcorridor 
plot_daily_ridership <- function(df, id) {
  a <- as.data.frame(subset(df$route_analytics, subcorridor_id==id))
  p <-  ggplot(a, aes(y = daily_ridership, x = route_id, label = daily_ridership)) + 
    geom_bar(stat = "identity", fill = "skyblue") +
    geom_text(position=position_dodge(width=0.9), vjust = -0.75, size = 3) +
    scale_y_continuous(name = "Total Riders Served", n.breaks = 8, limits = c(0, max(a$daily_ridership) * 1.1)) + 
    scale_x_discrete(name = "Route Number") + 
    labs(title = paste0("The ",  subset(df$subcorridor_name, subcorridor_id==id), " Sub-Corridor Serves ", round(sum(a$daily_ridership)), " Riders per Day")) +
    theme(text = element_text(size = 9))
 return(p)
  
}

plot_daily_ridership_trips <- function(df,id){
  a <- as.data.frame(subset(df$route_analytics, subcorridor_id==id))
  grid.arrange(ncol=2,
  ggplot(a, aes(y = daily_ridership, x = route_id, label = daily_ridership)) + 
    geom_bar(stat = "identity", fill = "skyblue") +
    geom_text(position=position_dodge(width=0.9), vjust = -0.75, size = 3) +
    scale_y_continuous(name = "Total Riders Served", n.breaks = 8, limits = c(0, max(a$daily_ridership) * 1.1)) + 
    scale_x_discrete(name = "Route Number") + 
    ...
    theme(text = element_text(size = 9)),
  ggplot(a, aes(x=route_id, y = trips, label = trips)) + 
    geom_bar(position="dodge", stat="identity") + 
    geom_text(position=position_dodge(width=0.9), vjust = -0.75, size = 3) +
    ylab("Daily Trips (2019)") + 
    scale_x_discrete(name = "Route Number") + 
    scale_fill_phl(palette = "main", discrete = T) +
    labs(fill= "", title = paste0("The Corridor Serves ", sum(a$trips, na.rm = TRUE), " Trips per Day")) + 
    #theme_phl() + 
...
plot_ridership_by_route <- function(df, id) {
  a <- as.data.frame(subset(df$hourly_route_analytics, subcorridor_id==id))
  ggplot(a, aes(x = trip_hour, y = daily_ridership, group=route_id, fill=route_id)) + 
    geom_bar(position = "stack", stat = "identity") +
...
plot_speed_by_period <- function(df,id) {
  a <- as.data.frame(subset(df$binned_analytics, subcorridor_id==id))
  p <- ggplot(a, aes(x = timeframe, y = avg_segment_speed)) + 
...
plot_ridership_by_period <- function(df,id) {
  a <- as.data.frame(subset(df$binned_analytics, subcorridor_id==id))
  p <- ggplot(a, aes(x = timeframe, y = daily_ridership, label = daily_ridership)) + 
      geom_bar(stat = "identity", fill="skyblue", alpha=0.7) +
      geom_text(position=position_dodge(width=0.9), vjust = -0.75, size = 3) +
...
plot_service_hrs <- function(df,id) {
  a <- as.data.frame(subset(df$route_analytics, subcorridor_id==id))
  p <- ggplot(a, aes(x=route_id, y = service_hours, label = round(service_hours))) + 
    geom_bar(position="dodge", stat="identity") + 
    ...
    scale_x_discrete(name = "Route Number") + 
    scale_fill_phl(palette = "main", discrete = T) +
    labs(fill= "", title = paste0("The Corridor Serves ", round(sum(a$service_hours, na.rm = TRUE), 1), " Service Hours per Day")) + 
    #theme_phl() + 
    theme(legend.position = "top")+
    theme(text = element_text(size = 9))
  return(p)
...
plot_running_time <- function(df,id) {
  a <- as.data.frame(subset(df$hourly_route_direction_analytics, subcorridor_id==id))
  p <- ggplot(a %>% subset(avg_run >=0), aes(x = trip_hour, y = avg_run, fill=direction_id)) + 
    geom_bar(position = "stack", stat = "identity") +
    #geom_point(size=3) + 
...
stop_list <- I(list(c("SEPTA372","SEPTA15911","SEPTA15794", "SEPTA15912", "SEPTA15789", "SEPTA15915", "SEPTA15586", "SEPTA15793", "SEPTA15913","SEPTA15791", "SEPTA15914", "SEPTA15792", "SEPTA15786", "SEPTA15916", "SEPTA15782", "SEPTA15917", "SEPTA15587", "SEPTA16979", "SEPTA15779", "SEPTA15918", "SEPTA15919","SEPTA15796", "SEPTA15908", "SEPTA15798", "SEPTA15907", "SEPTA15799", "SEPTA15800", "SEPTA16966", "SEPTA15814", "SEPTA16965", "SEPTA381", "SEPTA15795", "SEPTA373", "SEPTA15910","SEPTA16963", "SEPTA15815", "SEPTA16964", "SEPTA15817","SEPTA15819", "SEPTA16961", "SEPTA32312", "SEPTA16960", "SEPTA15820", "SEPTA16959", "SEPTA15822", "SEPTA15697", "SEPTA15899", "SEPTA15818", "SEPTA16962"),c("SEPTA372","SEPTA15911","SEPTA15794", "SEPTA15912", "SEPTA15789", "SEPTA15915", "SEPTA15586", "SEPTA15793", "SEPTA15913","SEPTA15791", "SEPTA15914", "SEPTA15792", "SEPTA15786", "SEPTA15916", "SEPTA15782", "SEPTA15917", "SEPTA15587", "SEPTA16979", "SEPTA15779", "SEPTA15918", "SEPTA15919"),c("SEPTA15796", "SEPTA15908", "SEPTA15798", "SEPTA15907", "SEPTA15799", "SEPTA15800", "SEPTA16966", "SEPTA15814", "SEPTA16965", "SEPTA381", "SEPTA15795", "SEPTA373", "SEPTA15910"),c("SEPTA16963", "SEPTA15815", "SEPTA16964", "SEPTA15817","SEPTA15819", "SEPTA16961", "SEPTA32312", "SEPTA16960", "SEPTA15820", "SEPTA16959", "SEPTA15822", "SEPTA15697", "SEPTA15899", "SEPTA15818", "SEPTA16962")))
subcorridor_name <- c("Olney","Olney and Chelten - Olney and Broad", "Olney and Broad - Olney and 7th", "Olney and 7th - Olney and Front")
subcorridor_id <- c("OL","CB","B7","7F")
...
# trying to map plots to subcorridors_dat -- think I messed up with the structure of the functions or something
subcorridors_results <- subcorridors_dat %>% 
  mutate(daily_ridership = map(subcorridors_dat, plot_daily_ridership))
...
plot_daily_ridership(subcorridors_dat, "B7")

plot_hourly_speed(subcorridors_dat, "B7")

plot_speed_by_route_dir(subcorridors_dat,"B7")
...
plot_daily_ridership(subcorridors_dat, "7F")

plot_hourly_speed(subcorridors_dat, "7F")

plot_speed_by_route_dir(subcorridors_dat,"7F")
