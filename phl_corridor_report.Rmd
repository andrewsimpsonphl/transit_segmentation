...
ggplot(route_analytics, aes(x = route_id, y = avg_segement_speed)) + 
  ...
  scale_y_continuous(name = "Speed (MPH) - Includes Dwell Time", n.breaks = 8, limits = c(0, max(route_analytics$adj_speed_90_pct) * 1.1)) +
  scale_x_discrete(name = "Route Number") + 
  labs(title = paste0("The Average Bus Travels at ", round(mean(analytics$avg_segement_speed), 1), " MPH on the Corridor")) +
  theme(text = element_text(size = 9))
```
...
ggplot(binned_analytics, aes(x = timeframe, y = avg_segement_speed)) + 
  geom_bar(stat = "identity", fill="skyblue", alpha=0.7) +
  ...
  scale_y_continuous(name = "Speed (MPH) - Includes Dwell Time", n.breaks = 8, limits = c(0, max(binned_analytics$adj_speed_90_pct) * 1.1)) +
  scale_x_discrete(name = element_blank()) +
...
ggplot(hourly_analytics) + 
  ...
  geom_point(aes(x = trip_hour, y = v), shape=21, size=3) + 
