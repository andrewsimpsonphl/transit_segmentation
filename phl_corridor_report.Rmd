...
ggplot(route_analytics, aes(x = route_id, y = avg_adj_speed)) + 
  geom_bar(stat = "identity", fill="skyblue", alpha=0.7) +
  geom_errorbar(data = route_analytics, stat = "identity", ymin = route_analytics$adj_speed_10_pct, ymax = route_analytics$adj_speed_90_pct, colour="orange", alpha=0.6, size=1.3, width = 0.4) + 
  scale_y_continuous(name = "Speed (MPH) - Excludes Dwell Time", n.breaks = 8, limits = c(0, max(route_analytics$adj_speed_90_pct) * 1.1)) +
  scale_x_discrete(name = "Route Number") + 
  labs(title = paste0("The Average Bus Travels at ", round(mean(analytics$avg_adj_speed), 1), " MPH on the Corridor")) +
  theme(text = element_text(size = 9))
```
...
ggplot(binned_analytics, aes(x = timeframe, y = avg_adj_speed)) + 
  geom_bar(stat = "identity", fill="skyblue", alpha=0.7) +
  geom_errorbar(data = binned_analytics, stat = "identity", ymin = binned_analytics$adj_speed_10_pct, ymax = binned_analytics$adj_speed_90_pct, colour="orange", alpha=0.6, size=1.3, width = 0.4) + 
  scale_y_continuous(name = "Speed (MPH) - Excludes Dwell Time", n.breaks = 8, limits = c(0, max(binned_analytics$adj_speed_90_pct) * 1.1)) +
  scale_x_discrete(name = element_blank()) +
...
ggplot(hourly_route_analytics, aes(x = trip_hour, y = avg_adj_speed, group=route_id, color=route_id)) + 
  geom_line() +
...
ggplot(hourly_route_direction_analytics, aes(x = trip_hour, y = avg_adj_speed, color=direction_id)) + 
  geom_line() +
  geom_point(size=1.5) + 
  #scale_color_viridis(discrete = TRUE) +
  scale_y_continuous(name = "Speed (MPH) - Excludes Dwell Time", n.breaks = 8) +
